...
- Run the normal Windows `check` mode once per selected node. The manual precision workflow retains ten repeats.
...
- Do not increase normal Windows check repeats beyond one.

## Data Flow

1. Each runner copies the canonical node-ID file into `tmp/precision-results/selected-nodeids.txt`.
2. The Linux/macOS runner executes each node once or ten times according to its current precision configuration; the Windows normal check executes once and precision triage keeps ten.
