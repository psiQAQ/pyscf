...
$RunIdsJson = ConvertTo-Json -InputObject @($WindowsRunId.ToString([Globalization.CultureInfo]::InvariantCulture),$LinuxRunId.ToString([Globalization.CultureInfo]::InvariantCulture)) -Compress
$RunIdsJsonBase64 = [Convert]::ToBase64String([Text.UTF8Encoding]::new($false).GetBytes($RunIdsJson))
$Args = @('-NoProfile','-ExecutionPolicy','Bypass','-File',$SupervisorPath,'-TargetRunIdsJsonBase64',$RunIdsJsonBase64,'-TargetHeadSha',$Head,'-HandoffPath',$HandoffPath,'-TerminalPath',$TerminalPath,'-Purpose','production-uhf-pair-heartbeat','-LaunchReadyPath',$LaunchReadyPath,'-LaunchReadySha256',$LaunchReadySha256,'-IntervalSeconds','1800','-WakeAfterMinutes','300')
...
if(@($args).Count-ne0){throw 'unexpected positional arguments'}
...
try{$RunIdsBytes=[Convert]::FromBase64String($TargetRunIdsJsonBase64);$RunIdsText=$Strict.GetString($RunIdsBytes);$RunIdsObject=$RunIdsText|ConvertFrom-Json}catch{throw "run-ID transport invalid: $($_.Exception.Message)"};if($RunIdsObject-isnot[array]-or@($RunIdsObject).Count-lt1-or@($RunIdsObject).Count-gt2){throw 'run-ID transport must be a one-or-two-item JSON array'};$TargetRunIds=@();foreach($Item in @($RunIdsObject)){if($Item-isnot[string]){throw 'run ID must be a JSON string'};$Parsed=0L;if(-not[long]::TryParse($Item,[Globalization.NumberStyles]::None,[Globalization.CultureInfo]::InvariantCulture,[ref]$Parsed)-or$Parsed-le0-or$Item-cne$Parsed.ToString([Globalization.CultureInfo]::InvariantCulture)-or$TargetRunIds-ccontains$Item){throw 'run ID must be positive, canonical and unique'};$TargetRunIds+=$Item};if($TransportSelfTest){[pscustomobject]@{target_run_ids=@($TargetRunIds);positional_args_count=@($args).Count;github_calls=0;goal_calls=0;heartbeat_processes_started=0}|ConvertTo-Json -Compress;exit 0}
if($TargetHeadSha-notmatch'^[0-9a-f]{40}$'-or[string]::IsNullOrWhiteSpace($Purpose)-or$LaunchReadySha256-notmatch'^[0-9a-f]{64}$'){throw 'supervisor arguments invalid'}
...
Normalize it with the same strict UTF-8/no-BOM/LF policy used for the tracked files. In a fresh shell run the exact harmless command `Set-Location -LiteralPath 'D:\workspace\pyscf\.worktrees\libxc-712-release-revalidation'; & 'D:\workspace\pyscf\.worktrees\libxc-712-release-revalidation\.superpowers\sdd\2026-08-11-sgx-extra-cycle-convergence-production\heartbeat-direct-handle-supervisor.ps1' -SelfTest; if (-not $?) { throw 'supervisor self-test failed' }`; require one JSON object with `wait_completed=true`, `direct_handle_has_exited=true`, `exit_code=0`, and `cim_count=0`. Then run the real cross-process transport probe below. Both self-test switches return before heartbeat/Goal/GitHub production logic; production arguments remain forbidden.

```powershell
$ErrorActionPreference='Stop';$Design='D:\workspace\pyscf\.worktrees\libxc-712-release-revalidation';$SupervisorPath='D:\workspace\pyscf\.worktrees\libxc-712-release-revalidation\.superpowers\sdd\2026-08-11-sgx-extra-cycle-convergence-production\heartbeat-direct-handle-supervisor.ps1';$PowerShellExe='C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe';Set-Location -LiteralPath $Design
$Leaf='libxc-712-ci-heartbeat.ps1';$Before=@(Get-CimInstance Win32_Process -ErrorAction Stop|Where-Object{[int]$_.ProcessId-ne$PID-and$_.CommandLine-and$_.CommandLine.IndexOf($Leaf,[StringComparison]::OrdinalIgnoreCase)-ge0});if($Before.Count-ne0){throw 'transport probe requires no existing heartbeat poller'};$Json=ConvertTo-Json -InputObject @('101','102') -Compress;$B64=[Convert]::ToBase64String([Text.UTF8Encoding]::new($false).GetBytes($Json));$OutputPath=Join-Path "$Design\tmp" "round2-run-id-transport-$PID-$([guid]::NewGuid().ToString('N')).json";$P=Start-Process -FilePath $PowerShellExe -WindowStyle Hidden -PassThru -RedirectStandardOutput $OutputPath -ArgumentList @('-NoProfile','-ExecutionPolicy','Bypass','-File',$SupervisorPath,'-TargetRunIdsJsonBase64',$B64,'-TransportSelfTest');if(-not$P.WaitForExit(30000)){throw 'two-ID transport probe timeout'};$P.Refresh();if(-not$P.HasExited){throw 'two-ID transport probe child remains'};for($I=0;$I-lt20-and@(Get-CimInstance Win32_Process -Filter "ProcessId=$([int]$P.Id)" -ErrorAction Stop).Count-ne0;$I++){Start-Sleep -Milliseconds 250};if(@(Get-CimInstance Win32_Process -Filter "ProcessId=$([int]$P.Id)" -ErrorAction Stop).Count-ne0){throw 'two-ID transport probe CIM row remains'};$R=([IO.File]::ReadAllText($OutputPath,[Text.UTF8Encoding]::new($false,$true)).Trim()|ConvertFrom-Json);$After=@(Get-CimInstance Win32_Process -ErrorAction Stop|Where-Object{[int]$_.ProcessId-ne$PID-and$_.CommandLine-and$_.CommandLine.IndexOf($Leaf,[StringComparison]::OrdinalIgnoreCase)-ge0});if(@($R.target_run_ids).Count-ne2-or[string]$R.target_run_ids[0]-cne'101'-or[string]$R.target_run_ids[1]-cne'102'-or$R.positional_args_count-ne0-or$R.github_calls-ne0-or$R.goal_calls-ne0-or$R.heartbeat_processes_started-ne0-or$After.Count-ne0){throw 'two-ID cross-process transport proof mismatch'};[pscustomobject]@{target_run_ids=@($R.target_run_ids);positional_args_count=[int]$R.positional_args_count;github_calls=0;goal_calls=0;heartbeat_processes_started=0;cim_count=0}|ConvertTo-Json -Compress
```
...
Before approval, run this recovery/mutation probe, the direct-handle probe, and Task 3's real `Start-Process` two-ID transport probe. They do not read or write production state, call GitHub, or touch Goal state.
...
function Check-Exit([bool]$Leaf,[byte[]]$Bytes,[string]$Marker){if(-not$Leaf){throw 'exit leaf missing'};if(-not[Linq.Enumerable]::SequenceEqual([byte[]]$Bytes,[byte[]]@(0x30,0x0A))){throw 'exit bytes changed'};$A=[Security.Cryptography.SHA256]::Create();try{$Actual=[BitConverter]::ToString($A.ComputeHash($Bytes)).Replace('-','').ToLowerInvariant()}finally{$A.Dispose()};if($Actual-cne$Marker){throw 'exit marker differs from actual file'};$Actual};$Good=[byte[]]@(0x30,0x0A);$Sha='9a271f2a916b0b6ee6cecb2426f0b3206ef074578be55d9bc94f6f3fe3ab86aa';if((Check-Exit $true $Good $Sha)-cne$Sha){throw 'exit recovery GREEN failed'};$ExitFailures=0;try{[void](Check-Exit $false $Good $Sha)}catch{$ExitFailures++};try{[void](Check-Exit $true ([byte[]]@(0x31,0x0A)) $Sha)}catch{$ExitFailures++};if($ExitFailures-ne2){throw 'exit recovery mutation cases did not fail closed'}
[pscustomobject]@{initial='initialize';partial='resume';mismatch_failures=$Caught;fixed_order=$Actions;exit_recovery='PASS';exit_mutation_failures=$ExitFailures}|ConvertTo-Json -Compress
```
...
- [ ] Task 3 runs only materialization, the supervisor's harmless `-SelfTest`/`-TransportSelfTest`, and the harmless 19-case fence; its real `Start-Process` probe receives ordered IDs `101,102`, reports `$args=0`, and starts no heartbeat. Task 4 owns the non-overlapping macOS witness heartbeat; Task 5 starts the pair heartbeat only after witness child-gone/Goal `active` and invokes pair ordinals 1–4 in four fresh `shell_command` processes.
...
- [ ] The archive recovery probe returns `windows:complete,linux:resume`; missing and modified recovered exit-file fixtures both fail closed. Every recovered macOS/Linux completion reopens the actual ordinary `runner-exit-code.txt`, requires bytes `0x30 0x0A`, computes its current SHA and compares that value to the marker. Validator reports pass byte-preserving `Flush(true)` durability before any completion marker; all completion readbacks rehash their identity, run, validator, and report files.
